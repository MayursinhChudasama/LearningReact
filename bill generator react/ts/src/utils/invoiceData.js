export function invoiceDataFn(inputValues) {
  // console.log("inside invoiceDataFn");
  // console.log("value-at-start-inputValues", inputValues);

  // INVOICE NO
  function generateInvoiceNo(invoiceNoPrefix, invoiceNoStart, invoiceNoAdd) {
    if (invoiceNoPrefix && invoiceNoStart && invoiceNoAdd) {
      let invoiceNo = [];
      for (let i = 1; i < 15; i++) {
        invoiceNo.push(
  ...
  function generateParticulars(particulars) {
    if (particulars) {
      let finalArray = [];
      for (let i = 1; i < 17; i++) {
        let sortedParticulars = [...particulars].sort(
          (a, b) => Math.random() - 0.5
        );
        const arraySize =
  ...
  function generateQty(total, rates) {
    if (total && rates) {
  ...
  console.log("randomInputValueTotal", randomInputValueTotal);
